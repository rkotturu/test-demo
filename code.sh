...
cat /tmp/dir_before_commit_final | sort |uniq -c | awk -F " " '{print $2}' > /tmp/dir_before_commit_final
...
cat /tmp/dir_after_commit_final | sort |uniq -c | awk -F " " '{print $2}
' > /tmp/dir_after_commit_final
tr -s '[:blank:]' '[\n*]' </tmp/dir_after_commit_final |
